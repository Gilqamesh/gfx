...
# include "vulkan_debug_internal.c"
#endif
...
static bool vk__create_surface();
...
VkExtent2D swapchain__pick_extent(VkSurfaceCapabilitiesKHR* capabilities);
static bool vk__create_swapchain();
...
static bool vk__create_surface() {
    if (glfwCreateWindowSurface(vk.instance, window__get_glfw_window(vk.window), 0, &vk.surface) != VK_SUCCESS) {
        return false;
    }

    debug__write_and_flush(DEBUG_MODULE_VULKAN, DEBUG_INFO, "created vk surface on window '%s'", window__get_title(vk.window));
...
VkExtent2D swapchain__pick_extent(VkSurfaceCapabilitiesKHR* capabilities) {
    ...
    if (capabilities->currentExtent.width != UINT32_MAX) {
        ...
        glfwGetFramebufferSize(window__get_glfw_window(vk.window), (int32_t*) &framebuffer_width, (int32_t*) &framebuffer_height);
...
static bool vk__create_swapchain() {
    ...
    VkExtent2D sc_extent = swapchain__pick_extent(&swapchain_support_details.capabilities);
