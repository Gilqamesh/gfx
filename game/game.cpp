...
void game__update(game_t self, controller_t* window_controller, double s) {
    ...
    const float move_speed_per_second = 2.0f;
    ...
    if (controller__button_is_down(window_controller, BUTTON_A)) {
        self->position[0] -= move_speed;
    }
    if (controller__button_is_down(window_controller, BUTTON_D)) {
        self->position[0] += move_speed;
    }
    if (controller__button_is_down(window_controller, BUTTON_S)) {
        self->position[2] += move_speed;
    }
    if (controller__button_is_down(window_controller, BUTTON_W)) {
        self->position[2] -= move_speed;
    }
